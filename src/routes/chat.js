...
// ─── GET /api/online/last ─────────────────────────────────────
// Últimos usuários online (cadastrados + visitantes) para a tela de login
router.get('/online/last', async (req, res) => {
  try {
    const { rows } = await db.query(`
      WITH guests AS (
        SELECT DISTINCT ON (LOWER(nick)) nick, created_at AS last_seen
        FROM login_logs
        WHERE kind = 'guest' AND created_at > NOW() - INTERVAL '3 hours'
        ORDER BY LOWER(nick), created_at DESC
      ),
      combined AS (
        SELECT nick, role, avatar, photo_url, last_seen
        FROM users
        WHERE is_banned = FALSE AND last_seen IS NOT NULL
        UNION ALL
        SELECT g.nick, 'guest'::varchar AS role, '👤'::varchar AS avatar, NULL::text AS photo_url, g.last_seen
        FROM guests g
        WHERE NOT EXISTS (SELECT 1 FROM users u WHERE LOWER(u.nick) = LOWER(g.nick))
      )
      SELECT nick, role, avatar, photo_url
      FROM combined
  ...
  try {
    const { rows: [u] } = await db.query(
      `SELECT id, nick, role, avatar, photo_url, bio, city, age, gender, interests, job,
              ...
              cover_url, theme, mood, birthday, social_instagram, social_tiktok, social_twitter, social_youtube, social_facebook
