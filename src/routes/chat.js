...
// ─── ENQUETES (polls) em tempo real — schema do patch-06 ─────
async function pollCounts(id, optLen) {
  const { rows } = await db.query(
    'SELECT option_index, COUNT(*)::int AS n FROM poll_votes WHERE poll_id = $1 GROUP BY option_index', [id]
  );
  const counts = new Array(optLen).fill(0);
  rows.forEach(r => { if (r.option_index >= 0 && r.option_index < optLen) counts[r.option_index] = r.n; });
  return counts;
}
...
router.post('/polls', requireRole('supervisor'), async (req, res) => {
  try {
    const room_slug = (req.body.room_slug || 'geral').toString().slice(0, 50);
    ...
    const { rows: [p] } = await db.query(
      'INSERT INTO polls (room_slug, question, options, created_by) VALUES ($1, $2, $3, $4) RETURNING id',
      [room_slug, question, JSON.stringify(options), req.user.nick]
    );
...
// votar (um voto por usuário registrado)
router.post('/polls/:id/vote', authMiddleware, async (req, res) => {
  try {
    const id = parseInt(req.params.id, 10);
    const idx = parseInt(req.body.option_index, 10);
    if (!req.user.user_id) return res.status(403).json({ error: 'Faça login para votar.' });
    const { rows: [p] } = await db.query('SELECT is_active, options FROM polls WHERE id = $1', [id]);
    if (!p || !p.is_active) return res.status(400).json({ error: 'Enquete encerrada.' });
    const optLen = (p.options || []).length;
    if (isNaN(idx) || idx < 0 || idx >= optLen) return res.status(400).json({ error: 'Opção inválida.' });
    await db.query(
      'INSERT INTO poll_votes (poll_id, user_id, option_index) VALUES ($1, $2, $3) ON CONFLICT (poll_id, user_id) DO NOTHING',
      [id, req.user.user_id, idx]
    );
    const { rows: [mv] } = await db.query('SELECT option_index FROM poll_votes WHERE poll_id = $1 AND user_id = $2', [id, req.user.user_id]);
    const counts = await pollCounts(id, optLen);
    try { ws.broadcastAll({ event: 'poll_update', data: { id, counts } }); } catch (e) {}
    res.json({ ok: true, counts, my_vote: mv ? mv.option_index : idx });
...
router.get('/polls/active', authMiddleware, async (req, res) => {
  try {
    const { rows: [p] } = await db.query('SELECT id, question, options FROM polls WHERE is_active = TRUE ORDER BY id DESC LIMIT 1');
    if (!p) return res.json({ poll: null });
    const counts = await pollCounts(p.id, (p.options || []).length);
    let my = null;
    if (req.user.user_id) {
      const { rows: [mv] } = await db.query('SELECT option_index FROM poll_votes WHERE poll_id = $1 AND user_id = $2', [p.id, req.user.user_id]);
      if (mv) my = mv.option_index;
    }
    res.json({ poll: { id: p.id, question: p.question, options: p.options }, counts, my_vote: my });
...
router.post('/polls/:id/close', requireRole('supervisor'), async (req, res) => {
  try {
    ...
    await db.query('UPDATE polls SET is_active = FALSE, closed_at = NOW() WHERE id = $1', [id]);
...
// encerrar a enquete ativa (painel admin)
router.post('/polls/close', requireRole('supervisor'), async (req, res) => {
  try {
    const { rows: [p] } = await db.query('SELECT id, options FROM polls WHERE is_active = TRUE ORDER BY id DESC LIMIT 1');
    if (p) {
